...
pub fn variant (self , variant : DescriptorCountA) -> & 'a mut W { self . bits (variant . into ()) } # [inline (always)]
pub fn bits (self , value : u8) -> & 'a mut W { self . w . bits = (self . w . bits & ! (0x07 << 0)) | ((value as u8 & 0x07) << 0) ; self . w } } # [doc = "Specifies if the coordinator is accepting guaranteed timeslot requests."]
# [derive (Clone , Copy , Debug , PartialEq)]
...
pub fn variant (self , variant : PermitA) -> & 'a mut W { self . bits (variant . into ()) } # [doc = "Set the value of the `Permit` field to `NotPermitted`"]
# [inline (always)]
...
pub fn bits (self , value : bool) -> & 'a mut W { self . w . bits = (self . w . bits & ! (0x01 << 7)) | ((value as u8 & 0x01) << 7) ; self . w } } impl R { # [inline (always)]
